...
- 每个绑定账号有自己的听歌进度和随机下次开听时间。Cron 每分钟：先给「已经听满一首」的账号上报 `play`，再从到期空闲账号里最多开 6 个新听（按 256KB Range 分页拉流 → `startplay`）。拉流有 12 秒截止，超时只记 `audio.partial`，仍会 `startplay`，避免一次读完整首 7–8MB 把 Worker 掐死。同一账号有未结束的歌时不会再开下一首。
