...
- 每个绑定账号有自己的听歌进度和随机下次开听时间。Cron 每分钟：先给「已经听满一首」的账号上报 `play`，再从到期空闲账号里最多开 6 个新听（按 256KB Range 分页拉完整首并丢弃响应体 → `startplay`）。同一账号有未结束的歌时不会再开下一首。上一分钟如果还在下载或上报，这一分钟直接退出（`work.skip`），不新开听；已经在模拟听歌等待的账号不受影响，到点仍会由原任务或之后抢到锁的 cron 上报。
